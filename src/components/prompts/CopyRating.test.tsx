...
const mocks = vi.hoisted(() => ({ signedIn: true, copy: vi.fn(), getText: vi.fn(), rating: vi.fn(), submit: vi.fn(), increment: vi.fn(), incrementView: vi.fn(), toast: vi.fn(), login: vi.fn() }));
...
vi.mock("@/services/supabase/prompts", () => ({
  getPromptText: mocks.getText,
  incrementCopyCount: mocks.increment,
  incrementViewCount: mocks.incrementView,
}));
...
describe("copy-to-rating entry points", () => {
  beforeEach(() => {
    ...
    mocks.getText.mockResolvedValue({ text: "A portrait", error: null });
    mocks.increment.mockResolvedValue(undefined);
    mocks.incrementView.mockResolvedValue(undefined);
  ...
  it.each(["card", "detail"] as const)("does not offer rating or count a failed %s copy", async view => {
    ...
    mount(view);
    fireEvent.click(screen.getByRole("button", { name: /copy prompt/i }));
    await waitFor(() => expect(mocks.toast).toHaveBeenCalledWith(expect.objectContaining({ title: "Couldn't copy the prompt" })));
  ...
  it("links a signed-in card viewer without suppressing a later copy invitation", async () => {
    mount("card");
    fireEvent.click(screen.getByRole("button", { name: "Not yet rated" }));
