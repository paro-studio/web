...
export function RatingInvitation({ signedIn, onRate, onSignIn, onDismiss }: {
  signedIn: boolean;
  onRate: () => void;
  onSignIn: () => void;
  onDismiss: () => void;
  ...
  return (
    <div role="status" className="mt-3 rounded border border-border bg-secondary/40 p-3 text-sm" onClick={event => event.stopPropagation()}>
      ...
      <button type="button" className="mt-2 text-gold underline underline-offset-4" onClick={signedIn ? onRate : onSignIn}>
        {signedIn ? "Rate this prompt" : "Sign in to rate"}
      </button>
    </div>
  );
}
