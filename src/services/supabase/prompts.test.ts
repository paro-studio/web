...
describe("prompts service", () => {
  ...
  describe("getAllPrompts", () => {
    it("returns normalized prompts with camelCase properties", async () => {
      ...
      expect(selectMock).toHaveBeenCalledWith(PROMPT_SELECT_COLUMNS);
  ...
  describe("getPrompt", () => {
    it("fetches single prompt enumerating PROMPT_SELECT_COLUMNS to exclude fts", async () => {
      const singleMock = vi.fn().mockResolvedValue({
        data: mockDbPrompts[0],
        error: null,
      });
