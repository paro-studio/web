...
vi.mock("@/services/supabase/likes", () => ({ setLike: mocks.toggleLike }));
vi.mock("@/services/supabase/saves", () => ({ setSave: mocks.toggleSave }));
vi.mock("@/services/supabase/follows", () => ({
  setFollow: mocks.toggleFollow, getFollowerCount: async () => 12, isFollowing: async () => true,
}));
...
describe("social state from server data", () => {
  ...
  it("restores a card's like and count when the service returns an error", async () => {
    ...
    await waitFor(() => expect(mocks.toggleLike).toHaveBeenCalledWith("viewer", prompt.id, false));
  ...
  it("restores a cached profile's follow status and follower count after a failed unfollow", async () => {
    ...
    await waitFor(() => expect(mocks.toggleFollow).toHaveBeenCalledWith("viewer", "creator", false));
